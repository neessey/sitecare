...
export default function ClientsList({
 ...
 return (
  <div className="space-y-4 md:space-y-6">
    ...
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 bg-white p-3 sm:p-4 rounded-xl border border-zinc-200/80 shadow-xs">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
            ...
            {filteredClients.map((client) => {
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl border transition-all cursor-pointer hover:shadow-xs group
                    ${isClientSelected 
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-semibold text-zinc-900 group-hover:text-amber-700 transition-colors text-sm sm:text-base">
                          {client.nom}
                        </span>
                    ...
                    <div className="grid grid-cols-1 xs:grid-cols-2 gap-1.5 text-xs text-zinc-500">
                      ...
                      <span className="flex items-center gap-1 col-span-1 xs:col-span-2">
      ...
      {/* Selected client detailed view - responsive */}
      {selectedClient && (
        <div className="lg:col-span-2 bg-white rounded-xl border border-zinc-200/80 shadow-xs overflow-hidden flex flex-col h-full">
          {/* Header - responsive */}
          <div className="p-4 sm:p-6 bg-zinc-950 text-white relative">
            <button
              onClick={() => setSelectedClient(null)}
              className="absolute top-3 right-3 sm:top-4 sm:right-4 text-zinc-400 hover:text-white transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            <span className="text-[10px] sm:text-xs uppercase font-semibold tracking-widest text-amber-500 font-mono">
              Profil Client
            </span>
            <h2 className="text-xl sm:text-2xl font-bold mt-1">{selectedClient.nom}</h2>
            <p className="text-zinc-400 text-sm mt-0.5">{selectedClient.entreprise}</p>

            {/* Contact grid - fully responsive */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mt-4 sm:mt-6 text-xs text-zinc-300">
              <div className="flex items-center gap-2">
                <Phone className="w-4 h-4 text-amber-500 shrink-0" />
                <span className="truncate">Tél : {selectedClient.telephone}</span>
              </div>
              <div className="flex items-center gap-2">
                <MessageSquare className="w-4 h-4 text-emerald-400 shrink-0" />
                <span className="truncate">WhatsApp : {selectedClient.whatsapp || selectedClient.telephone}</span>
              </div>
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 sm:col-span-2">
                ...
                <span className="truncate">{selectedClient.adresse || 'Aucune adresse renseignée'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-6 flex-1 overflow-y-auto max-h-[300px] sm:max-h-[400px] md:max-h-[450px]">
            <div className="flex items-center gap-2 text-zinc-800 text-sm font-semibold mb-4 sm:mb-6">
              <History className="w-4 h-4 text-amber-500 shrink-0" />
              <span>Historique chronologique des événements</span>
            </div>

            <div className="relative border-l border-zinc-200 pl-4 sm:pl-6 space-y-4 sm:space-y-6">
              {getClientTimeline(selectedClient.id!).map((event, idx) => (
                <div key={idx} className="relative">
                  {/* Dot marker */}
                  <span className={`absolute -left-[21px] sm:-left-[31px] top-1.5 w-2.5 h-2.5 rounded-full border-2 border-white ring-4
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs text-zinc-400 mb-1">
                    <span className="font-mono bg-zinc-100 text-zinc-600 px-2 py-0.5 rounded-sm self-start sm:self-auto">
                      {event.date}
                    </span>
                    {event.amount && (
    ...
    {isAddModalOpen && (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 z-50">
        <div className="bg-white rounded-xl border border-zinc-200 shadow-xl max-w-md w-full max-h-[95vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-150">
          ...
          <form onSubmit={handleAddSubmit} className="p-4 sm:p-5 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <button
                ...
                className="flex-1 py-2 px-4 border border-zinc-200 text-zinc-600 rounded-lg hover:bg-zinc-50 transition-colors text-sm font-medium order-2 sm:order-1"
              >
                Annuler
              </button>
              <button
                type="submit"
                className="flex-1 py-2 px-4 bg-zinc-950 hover:bg-zinc-850 text-white rounded-lg transition-colors text-sm font-medium shadow-xs order-1 sm:order-2"
              >
                Enregistrer
              </button>
            </div>
    ...
    {/* Edit Client Modal - responsive (identique à add modal) */}
    {isEditModalOpen && (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 z-50">
        <div className="bg-white rounded-xl border border-zinc-200 shadow-xl max-w-md w-full max-h-[95vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-150">
          <div className="p-4 sm:p-5 border-b border-zinc-100 flex items-center justify-between bg-zinc-950 text-white sticky top-0 z-10">
