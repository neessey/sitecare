...
export default function ReportsView({ clients, projects, contracts, payments }: ReportsViewProps) {
 ...
 return (
  <div className="space-y-4 md:space-y-6" id="reports-view-section">
    {/* KPI Cards row - responsive */}
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 no-print">
      <div className="bg-white rounded-xl border border-zinc-200/80 p-4 sm:p-5 flex items-center justify-between shadow-xs">
        <div>
          <span className="text-[10px] uppercase font-bold text-zinc-400 font-mono">Part de la Vente</span>
          <h4 className="text-lg sm:text-xl font-bold font-mono text-zinc-900 mt-1">{siteRevenues.toLocaleString()} FCFA</h4>
          <p className="text-xs text-zinc-400 mt-1">Sites créés & livrés</p>
        </div>
        <div className="p-2.5 sm:p-3 bg-sky-50 rounded-xl text-sky-600 shrink-0">
      ...
      <div className="bg-white rounded-xl border border-zinc-200/80 p-4 sm:p-5 flex items-center justify-between shadow-xs">
        <div>
          ...
          <h4 className="text-lg sm:text-xl font-bold font-mono text-zinc-900 mt-1">{maintenanceRevenues.toLocaleString()} FCFA</h4>
      ...
      <div className="sm:col-span-2 lg:col-span-1 bg-zinc-950 text-white rounded-xl p-4 sm:p-5 flex items-center justify-between shadow-sm">
        <div>
          <span className="text-[10px] uppercase font-bold text-amber-500 font-mono">Chiffre d'Affaires</span>
          <h4 className="text-lg sm:text-xl font-black font-mono text-white mt-1">{totalRevenues.toLocaleString()} FCFA</h4>
          <p className="text-xs text-zinc-400 mt-1">Revenu brut cumulé</p>
        </div>
        <div className="p-2.5 sm:p-3 bg-amber-500 rounded-xl text-zinc-950 shrink-0">
          <TrendingUp className="w-5 sm:w-6 h-5 sm:h-6" />
        </div>
      </div>
    </div>

    {/* Action panel to export reports - responsive */}
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 bg-white p-3 sm:p-4 rounded-xl border border-zinc-200/80 shadow-xs no-print">
      <div>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 border border-zinc-200 hover:bg-zinc-50 text-[11px] sm:text-xs font-semibold text-zinc-700 rounded-lg transition-all"
          title="Exporter Répertoire Clients au format Excel / CSV"
        >
          <Table className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-amber-500" />
          <span className="hidden xs:inline">Export Clients</span>
        ...
        >
          <FileDown className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-amber-500" />
          <span className="hidden xs:inline">Export Paiements</span>
          <span className="xs:hidden">Paiements</span>
        </button>

        <button
          onClick={handlePrint}
          className="inline-flex items-center justify-center gap-1.5 px-3.5 py-1.5 bg-zinc-950 hover:bg-zinc-800 text-[11px] sm:text-xs font-semibold text-white rounded-lg transition-all"
          title="Imprimer le rapport ou générer un PDF"
        >
          <Printer className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-amber-400" />
          <span className="hidden xs:inline">Imprimer / PDF</span>
          <span className="xs:hidden">Imprimer</span>
        </button>
      </div>
    </div>

    {/* Grid split of monthly & annual aggregated details - responsive */}
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
      ...
      <div className="lg:col-span-2 bg-white rounded-xl border border-zinc-200/80 shadow-xs p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 no-print">
          ...
          <select
            ...
            className="w-full sm:w-auto px-2.5 py-1.5 text-xs font-semibold border border-zinc-200 rounded-lg bg-zinc-50"
          >
        ...
        <div className="overflow-x-auto -mx-4 sm:mx-0">
          <table className="w-full text-left text-xs min-w-[400px] sm:min-w-full">
            <thead>
              <tr className="bg-zinc-50/50 text-zinc-500 font-bold border-b border-zinc-100 font-mono uppercase">
                <th className="p-2 sm:p-3 text-left">Mois</th>
                <th className="p-2 sm:p-3 text-right">Vente Sites</th>
                <th className="p-2 sm:p-3 text-right">Maintenance</th>
                <th className="p-2 sm:p-3 text-right">Recette Totale</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100 text-zinc-700 font-medium">
              {monthlyData.map(m => (
                <tr key={m.index} className="hover:bg-zinc-50/50 transition-colors">
                  <td className="p-2 sm:p-3 text-zinc-900 font-semibold text-[11px] sm:text-xs">{m.name}</td>
                  <td className="p-2 sm:p-3 text-right font-mono text-zinc-600 text-[11px] sm:text-xs">
                    {m.site > 0 ? `${m.site.toLocaleString()} FCFA` : '-'}
                  </td>
                  <td className="p-2 sm:p-3 text-right font-mono text-emerald-600 text-[11px] sm:text-xs">
                    {m.maintenance > 0 ? `${m.maintenance.toLocaleString()} FCFA` : '-'}
                  </td>
                  <td className="p-2 sm:p-3 text-right font-bold font-mono text-zinc-950 text-[11px] sm:text-xs">
      ...
      {/* Annual aggregate overview - responsive */}
      <div className="bg-white rounded-xl border border-zinc-200/80 shadow-xs p-4 sm:p-6">
        ...
        <div className="space-y-3 sm:space-y-4 max-h-[400px] sm:max-h-[500px] overflow-y-auto pr-1">
          {annualData.map(y => (
            <div key={y.year} className="p-3 sm:p-4 bg-zinc-50/50 border border-zinc-200/60 rounded-xl space-y-2">
