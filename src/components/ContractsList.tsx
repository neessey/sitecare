...
export default function ContractsList({
 ...
 return (
  <div className="space-y-4 md:space-y-6">
    {/* Search and Filters panel - responsive */}
    ...
    {/* Grid workspace - responsive cards */}
    {filteredContracts.length === 0 ? (
      <div className="bg-white rounded-xl border border-zinc-200/80 p-8 sm:p-12 text-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
        {filteredContracts.map((contract) => {
          ...
          return (
            <div
              key={contract.id}
              className={`bg-white rounded-xl border transition-all p-4 sm:p-5 flex flex-col justify-between hover:shadow-md
            ...
            >
              <div>
                ...
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="flex items-start gap-2 min-w-0">
                    ...
                    `}>
                      {contract.statut === 'Actif' ? (
                        rInfo.overdue ? <ShieldAlert className="w-4 sm:w-5 h-4 sm:h-5" /> : <Shield className="w-4 sm:w-5 h-4 sm:h-5" />
                      ) : (
                        <ShieldOff className="w-4 sm:w-5 h-4 sm:h-5" />
                      )}
                    </div>
                    <div className="min-w-0">
                      <h4 className="font-bold text-sm text-zinc-900 leading-tight truncate">
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4 my-3 sm:my-4 p-2.5 sm:p-3 bg-zinc-50 rounded-xl border border-zinc-150/50 text-center">
                  <div>
                    <span className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wide block">Abonnement</span>
                    <span className="font-bold text-sm sm:text-base text-zinc-900 font-mono mt-0.5 block">
                  ...
                  <div>
                    ...
                    <span className="font-medium text-[11px] sm:text-xs text-zinc-800 mt-1 block">
                      {contract.frequence}
                    </span>
                  </div>
                </div>

                {/* Deadline info - responsive */}
                <div className="space-y-1.5 sm:space-y-2 mb-3 sm:mb-4">
                  <div className="flex flex-wrap items-center justify-between gap-1 text-xs">
                    <span className="text-zinc-500 font-medium">Prochaine échéance :</span>
                    <span className="font-mono text-zinc-950 font-bold text-[11px] sm:text-xs">
                      {contract.prochaineEcheance}
                    </span>
                  </div>
                  
                  <div className="flex flex-wrap items-center justify-between gap-1 text-xs">
                    <span className="text-zinc-500 font-medium">Statut :</span>
                    <span className={`px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-semibold ${rInfo.color}`}>
                      {rInfo.text}
                    </span>
                  </div>
                </div>
              ...
              <div className="pt-3 sm:pt-4 border-t border-zinc-100 flex flex-wrap items-center justify-between gap-2 mt-1">
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {contract.statut === 'Actif' ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2.5 sm:px-3 py-1.5 border border-amber-200 bg-amber-50/60 hover:bg-amber-100 text-amber-800 text-[10px] sm:text-xs font-semibold rounded-lg transition-colors"
                      title="Suspendre"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2.5 sm:px-3 py-1.5 border border-emerald-200 bg-emerald-50/60 hover:bg-emerald-100 text-emerald-800 text-[10px] sm:text-xs font-semibold rounded-lg transition-colors"
                      title="Réactiver"
                    >
                ...
                {contract.statut === 'Actif' && (
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-2.5 sm:px-3 py-1.5 bg-zinc-950 hover:bg-zinc-800 text-white text-[10px] sm:text-xs font-semibold rounded-lg transition-colors shadow-xs"
                  >
                    <DollarSign className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-amber-400" />
                    <span className="hidden xs:inline">Régler</span>
                    <span className="xs:hidden">Payer</span>
                  </button>
                )}
              </div>
            </div>
    ...
    {/* 1. Log Payment Modal - responsive */}
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 z-50">
        <div className="bg-white rounded-xl border border-zinc-200 shadow-xl max-w-md w-full max-h-[95vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-150">
          <div className="p-4 sm:p-5 border-b border-zinc-100 flex items-center justify-between bg-zinc-950 text-white sticky top-0 z-10">
            <div>
              <span className="text-[10px] text-emerald-400 uppercase tracking-widest font-mono font-bold">Maintenance</span>
          ...
          <form onSubmit={handleLogPaymentSubmit} className="p-4 sm:p-5 space-y-4">
            <div className="p-3 bg-emerald-50/50 border border-emerald-100 rounded-lg text-xs space-y-1">
              <p className="font-semibold text-emerald-950 truncate">
                Maintenance de : {projects.find(p => p.id === activeContractForPayment.projetId)?.nom}
              </p>
              <p className="text-zinc-600 truncate">Client : {clients.find(c => c.id === activeContractForPayment.clientId)?.nom}</p>
              <p className="text-zinc-600">Échéance : <span className="font-bold text-zinc-900">{activeContractForPayment.prochaineEcheance}</span></p>
            </div>
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 sm:gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-zinc-600">Moyen *</label>
                <select
                  value={paymentMethod}
            ...
            <div className="space-y-1.5">
              ...
              <input
                type="text"
                placeholder="Ex : Référence Wave, ID virement..."
                value={paymentRef}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-600">Observations</label>
              <textarea
            ...
            <div className="p-3 bg-amber-50 rounded-lg border border-amber-200 flex gap-2">
              ...
              <p className="text-[10px] text-amber-800 leading-normal font-medium">
                <strong>RENOUVELLEMENT AUTO :</strong> Le système calculera la nouvelle échéance selon la fréquence ({activeContractForPayment.frequence}).
              </p>
            </div>
            ...
            <div className="flex flex-col xs:flex-row gap-3 pt-1">
              <button
                ...
                className="flex-1 py-2 px-4 border border-zinc-200 text-zinc-600 rounded-lg hover:bg-zinc-50 transition-colors text-sm font-medium order-2 xs:order-1"
              >
                Annuler
              </button>
              <button
                type="submit"
                className="flex-1 py-2 px-4 bg-zinc-950 hover:bg-zinc-850 text-white rounded-lg transition-colors text-sm font-medium shadow-xs order-1 xs:order-2"
              >
                Confirmer
              </button>
            </div>
    ...
    {/* 2. Edit Contract Details Modal - responsive */}
    {isEditModalOpen && contractToEdit && (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 z-50">
        <div className="bg-white rounded-xl border border-zinc-200 shadow-xl max-w-md w-full max-h-[95vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-150">
          <div className="p-4 sm:p-5 border-b border-zinc-100 flex items-center justify-between bg-zinc-950 text-white sticky top-0 z-10">
            <h3 className="text-sm sm:text-base font-semibold">Paramètres du contrat</h3>
          ...
          <form onSubmit={handleEditSubmit} className="p-4 sm:p-5 space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-600">Prochaine échéance</label>
              <input
                type="date"
